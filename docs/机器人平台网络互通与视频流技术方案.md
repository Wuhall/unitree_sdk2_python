...
graph TD
    subgraph Platform Side
        A[User Browser/Client] -->|Control Command| B[Platform Backend];
        B -->|Publish Command| C[MQTT Broker];
        A -->|WebRTC Signaling| B;
        A -->|WebRTC Media Stream| H[WebRTC TURN Server];
    end
    ...
    subgraph Robot Side
        D[Robot Agent] -->|Subscribe Command| C;
        D -->|Publish Status/Data| C;
        D -->|WebRTC Signaling| C;
        E[Robot Camera] -->|Video Frames| D;
        D -->|WebRTC Media Stream| H;
    end

    B -->|Subscribe Status| C;
    C -->|Push Command| D;
    C -->|Push Status| B;
    C -->|Relay Signaling| A&D;
